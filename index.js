	...
	try {
		...
		if (liveStream && (!isLiveNotified || liveStream.title !== lastLiveTitle)) {
			...
			const messages = [
				{
					type: "text",
					text: `🎉 配信開始！\n視聴はこちら: https://www.openrec.tv/live/${
						liveStream.id || ""
					}`,
				},
			];
			...
			// 配信が終了している場合にLINEへ通知
			const messages = [{ type: "text", text: "📹 配信が終了しました。" }];
			await sendLinePushMessage(messages);
		...
		console.error(
			`[${getCurrentFormattedDate()}] ❌ OpenREC APIエラー:`,
...
cron.schedule("*/1 * * * *", async () => {
	console.log(
		`[${getCurrentFormattedDate()}] 🕒 OpenRECの配信状況をチェック中...`
	);
...
app.listen(PORT, () => {
	console.log(
		`[${getCurrentFormattedDate()}] 🚀 サーバーがポート ${PORT} で起動しました`
	);
});
